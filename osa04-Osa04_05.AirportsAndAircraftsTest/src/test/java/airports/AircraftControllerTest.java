...
public class AircraftControllerTest {
    ...
    public void nameExists() throws Exception {
        ...
        boolean isInDB = aircrafts.stream()
                       .anyMatch(port -> port.getName().equals(name));
        
        assertTrue(isInDB);
    }
    
    @Ignore("Tämä testi on skipattu koska mallivastauksissa ei ole ohjelman logiikkaa muuttavia vastauksia")
    @Test
    public void aircraftXP55Exists() throws Exception {
        ...
        boolean isInModel = aircrafts.stream()
                       .anyMatch(port -> port.getName().equals(name));
